...
import { FaWhatsapp, FaStar, FaUserCircle } from 'react-icons/fa'; // Added FaUserCircle
import { IoCloseCircleOutline, IoMenu } from 'react-icons/io5'; // Added IoMenu
...
export default function Home({ lemons }) {
    ...
    // New state for account sidebar
    const [isSidebarOpen, setIsSidebarOpen] = useState(false);
    const [accountDetails, setAccountDetails] = useState({ name: '', contact: '', delivery: '' });
    ...
    useEffect(() => {
        const savedAccount = localStorage.getItem('3lemons_account');
        if (savedAccount) {
            ...
            setForm(prevForm => ({
                ...prevForm,
                name: parsedAccount.name || prevForm.name,
                delivery: parsedAccount.delivery || prevForm.delivery,
                contact: parsedAccount.contact || prevForm.contact,
            }));
            setFeedback({ message: 'Welcome back! Your saved details have been loaded.', type: 'info' });
        }
    }, []);
    ...
    const saveAccountDetails = () => {
        ...
        if (!accountDetails.name.trim() || !accountDetails.contact.trim() || !accountDetails.delivery.trim()) {
            setFeedback({ message: 'Please fill in all account details.', type: 'error' });
            setSavingAccountDetails(false);
            return;
        }
        if (!/^\d{10}$/.test(accountDetails.contact)) {
            setFeedback({ message: 'Please enter a valid 10-digit contact number for your account.', type: 'error' });
            setSavingAccountDetails(false);
            return;
        }

        try {
            localStorage.setItem('3lemons_account', JSON.stringify(accountDetails));
            // Update the main form state with the saved details
            setForm(prevForm => ({
                ...prevForm,
                name: accountDetails.name,
                contact: accountDetails.contact,
                delivery: accountDetails.delivery
            }));
            setFeedback({ message: 'Account details saved and synced to form!', type: 'success' });
        } catch (error) {
            console.error('Error saving account details:', error);
            setFeedback({ message: 'Failed to save account details. Please try again.', type: 'error' });
        } finally {
            setSavingAccountDetails(false);
        }
    };
    ...
    const clearAccountDetails = () => {
        localStorage.removeItem('3lemons_account');
        setAccountDetails({ name: '', contact: '', delivery: '' });
        // Also clear corresponding fields in the main form
        setForm(prevForm => ({
            ...prevForm,
            name: '',
            contact: '',
        ...
        setFeedback({ message: 'Account details cleared.', type: 'info' });
    ...
    const handleSubmit = async (e) => {
        ...
        if (!form.name.trim() || !form.delivery.trim() || !form.contact.trim()) {
            setFeedback({ message: 'Please fill in all your personal details (Name, Delivery Address, Contact).', type: 'error' });
            return;
        }
        if (!/^\d{10}$/.test(form.contact)) {
            setFeedback({ message: 'Please enter a valid 10-digit contact number.', type: 'error' });
            return;
        }

        const validOrders = orders.filter(order => order.grade && order.quantity && parseInt(order.quantity) > 0);
        if (validOrders.length === 0) {
            setFeedback({ message: 'Please add at least one lemon variety with a valid quantity (must be 1 or more).', type: 'error' });
            return;
        }
        ...
        if (hasInvalidQuantity) {
            setFeedback({ message: 'Please ensure all selected varieties have a valid quantity (1 or more).', type: 'error' });
            return;
        }
        // --- End Validation ---
    ...
    const confirmAndSubmitOrder = async () => {
        ...
        if (!confirmedOrderDetails) { // Should not happen if flow is correct
            setFeedback({ message: 'Error: No order details to confirm.', type: 'error' });
        ...
        try {
            ...
            if (response.ok) {
                setShowSuccessModal(true); // Successfully submitted, show success modal
                ...
                setFeedback({ message: `Failed to submit order: ${errorData.message || 'Server error'}. Please try again.`, type: 'error' });
            }
        } catch (err) {
            console.error('Network or submission error:', err);
            setFeedback({ message: 'Failed to submit order. Please check your internet connection and try again.', type: 'error' });
        } finally {
            setIsSubmitting(false);
        }
    };
    ...
    const cancelConfirmation = () => {
        ...
        setFeedback({ message: 'Order confirmation cancelled.', type: 'info' });
    };
    ...
    return (
        <div className={styles.page}>
            ...
            {/* --- NEW: Header with Account Icon --- */}
            <header className={styles.header}>
                <h1 className={styles.headerTitle}>3 Lemons Traders</h1>
                <FaUserCircle className={styles.accountIcon} onClick={toggleSidebar} />
            </header>
            ...
            {/* --- NEW: Account Sidebar --- */}
            <div className={`${styles.accountSidebar} ${isSidebarOpen ? styles.open : ''}`}>
                ...
                <form className={styles.sidebarForm} onSubmit={(e) => { e.preventDefault(); saveAccountDetails(); }}>
                    <p className={styles.sidebarInfo}>Save your details for faster ordering!</p>
                    <div className={styles.sidebarFormGroup}>
                        <label className={styles.sidebarLabel} htmlFor="accountName">Name</label>
                        <input
                            id="accountName"
                            name="name"
                    ...
                    <div className={styles.sidebarFormGroup}>
                        <label className={styles.sidebarLabel} htmlFor="accountContact">Contact Number</label>
                        <input
                            id="accountContact"
                            name="contact"
                            className={styles.sidebarInput}
                    ...
                    <div className={styles.sidebarFormGroup}>
                        <label className={styles.sidebarLabel} htmlFor="accountDelivery">Delivery Address</label>
                        <input
                            id="accountDelivery"
                            name="delivery"
                            className={styles.sidebarInput}
                            type="text"
                            value={accountDetails.delivery}
                            onChange={handleAccountDetailChange}
                            placeholder="Your Delivery Address"
                            required
                        />
                    </div>
                    <button type="submit" className={styles.sidebarButton} disabled={savingAccountDetails}>
                        {savingAccountDetails ? 'Saving...' : 'Save & Sync Details'}
                    </button>
                    <button type="button" className={`${styles.sidebarButton} ${styles.cancel}`} onClick={clearAccountDetails} style={{ marginTop: '10px' }}>
                        Clear Saved Details
                    </button>
                </form>
            </div>
