...
import { FaWhatsapp, FaStar, FaUserCircle, FaEdit, FaHome, FaHistory, FaSignOutAlt, FaPlus, FaMinus, FaTrash, FaCheckCircle, FaExclamationCircle, FaInfoCircle, FaSpinner } from 'react-icons/fa';
...
// If your SheetDB is not set up, these will fail.
const LEMONS_DATA_URL = 'https://sheetdb.io/api/v1/wm0oxtmmfkndt?sheet=Lemons'; // Your Lemons data SheetDB URL
const ORDERS_SUBMISSION_URL = 'https://sheetdb.io/api/v1/wm0oxtmmfkndt?sheet=orders'; // Your Orders submission SheetDB URL
const SIGNUP_SHEET_URL = 'https://sheetdb.io/api/v1/wm0oxtmmfkndt?sheet=signup'; // Your Signup/Users SheetDB URL
const ADDRESSES_SHEET_URL = 'https://sheetdb.io/api/v1/wm0oxtmmfkndt?sheet=Addresses'; // Your Addresses SheetDB URL
...
export default function Home({ lemons }) {
    ...
    const saveAccountDetails = async (e) => { // Added 'e' parameter
        ...
        try {
            const response = await fetch(`${SIGNUP_SHEET_URL}/phone/${loggedInUser.phone}`, { // Use PUT for updating by phone
                method: 'PUT',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    data: {
    ...
    const fetchUserAddresses = useCallback(async (userPhone) => {
        ...
        try {
            ...
            if (Array.isArray(addresses)) {
                ...
                setUserAddresses(addresses.map(addr => ({ ...addr, id: addr.id || crypto.randomUUID() })));
            } else {
                setUserAddresses([]);
            }
        } catch (error) {
    ...
    const handleSaveAddress = async (e) => {
        ...
        try {
            ...
            if (id) { // Editing existing address
                response = await fetch(`${ADDRESSES_SHEET_URL}/id/${id}`, { // Assuming 'id' is present in SheetDB
                    method: 'PUT', // Use PUT for updating an existing row by ID
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ data: addressData }),
                });
            } else { // Truly new address
    ...
    const handleDeleteAddress = async (addressId) => {
        if (!window.confirm('Are you sure you want to delete this address?')) return; // Simple confirm for now
        setIsManagingAddresses(true);
        setFeedback({ message: '', type: '' });
        try {
...
// This function runs at build time or on demand with ISR
export async function getStaticProps() {
    try {
        const res = await fetch(LEMONS_DATA_URL); // Use the constant defined at the top
        if (!res.ok) {
            console.error(`Failed to fetch lemons: ${res.status} ${res.statusText}`);
            const errorBody = await res.text();
            console.error('Lemons API Response Body:', errorBody);
            // Return empty array and revalidate to try again later
            return { props: { lemons: [] }, revalidate: 30 };
        }
